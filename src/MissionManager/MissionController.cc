...
VisualMissionItem* MissionController::insertVtolTransitionItem(QGeoCoordinate coordinate, int visualItemIndex, bool transitionToFixedWing, bool makeCurrentItem)
{
    int sequenceNumber = _nextSequenceNumber();
    int transitionState = transitionToFixedWing ? MAV_VTOL_STATE_FW : MAV_VTOL_STATE_MC;

    SimpleMissionItem* vtolTransitionItem = qobject_cast<SimpleMissionItem*>(_insertSimpleMissionItemWorker(coordinate, MAV_CMD_DO_VTOL_TRANSITION, visualItemIndex, makeCurrentItem));
    vtolTransitionItem->setSequenceNumber(sequenceNumber);
    vtolTransitionItem->missionItem().setParam1(transitionState);
    _initVisualItem(vtolTransitionItem);

    if (visualItemIndex == -1) {
        _visualItems->append(vtolTransitionItem);
    } else {
        _visualItems->insert(visualItemIndex, vtolTransitionItem);
    }

    _recalcAll();
    if (makeCurrentItem) {
        setCurrentPlanViewSeqNum(vtolTransitionItem->sequenceNumber(), true);
    }
    _firstItemAdded();
    return vtolTransitionItem;
}

VisualMissionItem* MissionController::insertVtolTakeoffItem(QGeoCoordinate /*coordinate*/, int visualItemIndex, bool makeCurrentItem)
{
    int sequenceNumber = _nextSequenceNumber();
    _takeoffMissionItem = new TakeoffMissionItem(MAV_CMD_NAV_VTOL_TAKEOFF, _masterController, _flyView, _settingsItem, false /* forLoad */);
