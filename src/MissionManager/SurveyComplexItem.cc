...
void SurveyComplexItem::_rebuildTransectsPhase1WorkerSinglePolygon(bool refly)
{
    ...
    if (_flyAlternateTransectsFact.rawValue().toBool()) {
        ...
        // int skipCount = 5;  // Fly every fifth transect and skip the previous four
        // QList<QList<QGeoCoordinate>> skippedTransects;

        // // First pass: Fly every fifth transects
        // for (int i = 0; i < transects.count(); i += skipCount) {
        //     skippedTransects.append(transects[i]);
        // }

        // // Second pass: Fly the skipped transects
        // for (int i = 1; i < skipCount; i++) {
        //     for (int j = transects.count() - i; j >= 0; j -= skipCount) {
        //         skippedTransects.append(transects[j]);
        //     }
        // }

        // transects = skippedTransects;
    }
